...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      setMousePosition((prev) => {
        const speed = 0.3; // snappier
        const dx = targetPos.current.x - prev.x;
        const dy = targetPos.current.y - prev.y;
        const dist = Math.sqrt(dx * dx + dy * dy);

        // scale effect based on speed
        const velocity = Math.min(dist / 40, 0.5);
        setScale(1 + velocity);
  ...
  const blobStyle = {
    ...
    filter: "blur(80px)", // stronger glow
    ...
    borderRadius: "50%",
    backgroundColor: "#FFFB00",
    boxShadow: "0 0 60px 30px #FFFB00, 0 0 120px 60px #FFFB00", // glowing halo
    pointerEvents: "none",
  ...
  return (
    ...
    >
      ...
      <div className="z-10 text-center space-y-6 w-full">
        ...
        <div className="flex flex-wrap justify-center items-center gap-3 mt-4">
          <div className="skill-box">Listen</div>
          <div className="skill-box">Guess</div>
          <div className="skill-box">Win</div>
        </div>
